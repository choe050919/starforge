...
func query(cell: Vector2i):
	...
	if not is_ready():
		return _make_info(cell, -1, -1, -1, -1, -1)
	...
	var mass_val := _read_mass(cell)
	var temp_val := _read_temperature(cell)
	var light_val := _read_light(cell)

	return _make_info(cell, substance_val, phase_val, mass_val, temp_val, light_val)
...
func _read_light(cell: Vector2i) -> float:
	return _light_store.get_by_cell(cell)

func _make_info(cell: Vector2i, substance_val, phase_val: int, mass_val: int, temperature_val: int, light_val: float):
