...
# 허용 키(딱 5개): "sid" | "phase" | "mass" | "temp"
# 규칙:
# 반드시 "sid" 포함(없으면 거부).
# 나머지 키는 “선택적 오버라이드”. 없으면 스키마(TileSchema) 에서 기본값을 채움.
# VACUUM이면 mass=0, phase=VACUUM, sid=VACUUM, temp=0로 강제.
func apply_cells_with_spec(cells: Array[Vector2i], tile_spec: Dictionary) -> void:
	if not tile_spec.has(SID):
		push_error("[DataLayer] tile_spec must include 'sid'")

	var sid: int = tile_spec["sid"]

	# 1) 스키마에서 기본 스펙 가져오기 (tile-> {sid, phase, mass, temp})
	## 스키마에 저장된 기본 스펙
	var spec = get_spec(sid)  # {sid, phase, mass, temp}

# TODO 작업중 TODO 작업중 TODO 작업중 TODO 작업중 TODO 작업중 TODO 작업중 
...
"""
	...
	if spec <= 0:
		spec.mass = 0
		spec.sid = 0
		spec.phase = PhaseStore.Phase.VACUUM
		spec.temp = 0
	elif spec.phase == PhaseStore.Phase.VACUUM:
		spec.phase = _schema.get_default_phase_for_sid(spec.sid)

	# 4) 필요한 스토어만 begin
	var to_begin := []
	to_begin.append(tile_types)
	to_begin.append(substances)
	to_begin.append(phases)
	to_begin.append(mass)
	to_begin.append(temperatures)
	for s in to_begin: s.begin_write()

	# 5) 배치 set
	for c in cells:
		var i := index.key(c)
		tile_types.set_by_index(i, tile_id)
		substances.set_by_index(i, spec.sid)
		phases.set_by_index(i, spec.phase)
		mass.set_by_index(i, spec.mass)
		temperatures.set_by_index(i, spec.temp)

	# 6) commit
	for s in to_begin: s.commit_write()
"""
